...
if 1:#edit
    ...
    boxVectors = stateinfo.getPeriodicBoxVectors(asNumpy=True)[:]
    print boxVectors
    print boxVectors[0], boxVectors[1], boxVectors[2]
    barostat_index = systemFromContext.getNumForces() -1
    md_simulation.system.removeForce(barostat_index)
#    md_simulation.context.reinitialize()
#    md_simulation.context.setPositions(statepos)
    md_simulation.context.setPeriodicBoxVectors(boxVectors[0], boxVectors[1], boxVectors[2])
#    t = md.load('u_debug1.h5')
#    md_simulation.context.setPeriodicBoxVectors(t.openmm_boxes(-1))
#    md_simulation.reporters.append(HDF5Reporter('xxx_debug1.h5', 100))
    md_simulation.step(stepsNVT)
...
#    boxVectors = stateinfo.getPeriodicBoxVectors()[:]
    ...
    nvt_simulation = openmm.app.simulation.Simulation(topology=testsystem.topology, system=testsystem.system, integrator=nvt_integrator, state=temp_xml)
